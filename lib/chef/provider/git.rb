    ...
    class Git < Chef::Provider
          ...
          requirements.assert(:all_actions) do |a|
            a.assertion {
              begin
                ...
                get_dir(new_resource.user)
              rescue ArgumentError
                false
              end
            }
            a.whyrun("User #{new_resource.user} does not exist.")
            a.failure_message(Chef::Exceptions::User, "'#{new_resource.user}' for resource '#{new_resource.name}' does not exist")
          end
        end
      ...
      def run_options(run_opts = {})
        ...
        if new_resource.user
          ...
          env["HOME"] = get_dir(new_resource.user)
        end
